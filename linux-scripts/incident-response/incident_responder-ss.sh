...
if [[ "$WANT_EXCLUDE" =~ ^[Yy]$ ]]; then
    ...
    if ((${#names[@]} > 0)); then
        ...
        echo
        echo "[+] ss exclusion enabled for: ${names[*]}"
        echo
    else
        EXCLUDE_ENABLED=false
        echo
...
monitor_sessions_fast() {
    while true; do
        ...
        if [ -n "$NEW_SESSION" ]; then
            NS_TPEN="$(ss -tpen 2>/dev/null)"
            NS_TPLEN="$(ss -tplen 2>/dev/null)"
...
while true; do
    ...
    print_ss_excluding "ss -tplen" "Listening Ports"
    print_ss_excluding "ss -tpen" "Established Ports"
